...
if ($eror === 4) {
	$result = mysqli_query($conn, "UPDATE produk SET nama = '$nm_produk', deskripsi = '$desk', harga = '$harga', katalog = '$katalog', minimal_pemesanan='$minimal_pemesanan' where kode_produk = '$kode'");
	if ($result) {
		echo "
		<script>
...
if (file_exists("../../image/produk/" . $tgambar['image'])) {
	...
	$result = mysqli_query($conn, "UPDATE produk SET nama = '$nm_produk', image = '$namaGambarBaru' ,deskripsi = '$desk', harga = '$harga', katalog = '$katalog', minimal_pemesanan='$minimal_pemesanan' where kode_produk = '$kode'");
	if ($result) {
		echo "
		<script>
		alert('PRODUK BERHASIL DIEDIT');
